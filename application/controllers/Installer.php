...
class Installer extends MX_Controller
{
    public function __construct()
    {
        ...
        if ($this->_check_install() && $this->uri->segment(2) != 'done') {
            exit('Application already installed. Need help? https://hostingbilling.net');
        }
    ...
    public function verify()
    {
        ...
        if (json_decode($env_data) == null) {
            ...
            if ($p['buyer'] != $this->input->post('set_envato_user') && $p['buyer'] != 'codester') {
    ...
    public function _valid_script()
    {
        ...
        return remote_get_contents(UPDATE_URL.'verify/index.php?code='.$purchase_code);
    }
